...
import { ArrowLeft, Users, Check, Search, Image as ImageIcon } from 'lucide-react';
...
export const CreateGroupScreen: React.FC<{
  ...
  const { createGroupChat, isLoading, error: chatError, clearError } = useChatStore();
  ...
  const handleCreateGroup = async () => {
    if (!groupName.trim() || isLoading || submitting || submitLock.current) return;
    ...
    try {
      ...
      setLocalError('Could not create group. Try again.');
    } catch (err: any) {
      setLocalError(err?.message || chatError || 'Failed to create group.');
    } finally {
      setSubmitting(false);
      // keep lock briefly so a second tap in the same moment is dropped
      window.setTimeout(() => {
    ...
    <div className="w-full max-w-2xl mx-auto p-4 md:p-6 space-y-6 text-left pb-28">
      <div className="flex items-center gap-3">
        <button
          ...
          className="p-2 rounded-2xl bg-white/80 dark:bg-slate-800 hover:bg-white text-slate-700 dark:text-slate-200 border border-slate-200/80 dark:border-white/10 shadow-xs cursor-pointer transition-all"
        >
          <ArrowLeft size={18} />
        </button>
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-white">Create New Group</h1>
          <p className="text-xs text-slate-500 font-medium">Step {step} of 3</p>
        </div>
      </div>

      <div className="w-full bg-slate-200/60 dark:bg-slate-700 h-1.5 rounded-full overflow-hidden">
        <div
          ...
          <GlassCard className="p-6 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                rows={3}
                className="w-full p-3 rounded-2xl glass-input text-xs text-slate-800 dark:text-slate-100 font-medium focus:outline-none"
              />
            </div>
          </GlassCard>
      ...
      {step === 2 && (
        <div className="space-y-4">
          <GlassCard className="p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 p-3 bg-slate-50/80 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-white/10">
              <img
      ...
      {step === 3 && (
        <div className="space-y-4">
            ...
            <div className="max-h-60 overflow-y-auto space-y-2 pt-2">
              {searchResults.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-4">
                  Search for people on Relay to add them.
                </p>
              ) : (
                    ...
                    <div
                      ...
                      className={`p-3 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
                        isSel
                          ...
                          : 'bg-white/70 dark:bg-slate-800/50 border-slate-200/60 dark:border-white/10 hover:bg-white'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <img
                      ...
                      <div
                        className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                          isSel ? 'bg-blue-600 border-blue-600 text-white' : 'border-slate-300'
                        }`}
                      >
                        {isSel && <Check size={12} />}
                      </div>
          ...
          <div className="flex justify-between">
            <GlassButton onClick={() => setStep(2)} variant="secondary" className="py-2.5 px-5 text-xs" disabled={busy}>
              Back
            </GlassButton>
            <GlassButton
