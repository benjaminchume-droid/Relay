    ...
    <div className={`flex flex-col gap-1 p-1.5 rounded-xl min-w-[120px] max-w-[180px] ${
      ...
      {uploadStatus === 'failed' && (
        <div className="flex items-center justify-between text-xs text-rose-300 font-medium px-1 pb-1 border-b border-rose-500/30">
          ...
          {onRetryUpload && (
            <button onClick={onRetryUpload} className="px-2 py-0.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/40 text-white flex items-center gap-1 text-[10px] font-bold cursor-pointer">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 space-y-0.5 min-w-0">
              ...
              return (
                <div
                  ...
                  className={`w-0.5 rounded-full transition-all duration-75 ${
                    isPlayed
                      ? isMine ? 'bg-white shadow-xs' : 'bg-blue-600 shadow-xs'
                      : isMine ? 'bg-white/40 group-hover:bg-white/60' : 'bg-slate-300 dark:bg-slate-700 group-hover:bg-slate-400'
                  }`}
                />
              );
            })}
          ...
          <div className="flex items-center justify-between text-[9px] font-mono opacity-85">
            <span>
            ...
            <div className="flex items-center gap-1">
              <button type="button" onClick={(e) => { e.stopPropagation(); cycleSpeed(); }}
                className={`px-1 py-0.5 rounded-md text-[9px] font-bold transition-colors cursor-pointer ${
                  isMine ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 text-slate-700 dark:text-slate-300'
                }`}>{playbackSpeed}x</button>
              <button type="button" onClick={(e) => { e.stopPropagation(); downloadAudio(fileName); }}
                className={`p-0.5 rounded-md transition-colors cursor-pointer ${
                  isMine ? 'hover:bg-white/20 text-white/90' : 'hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-600'
